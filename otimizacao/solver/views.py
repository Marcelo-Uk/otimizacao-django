...
def parse_expression(expression, variables):
    try:
        ...
        constant_terms = re.findall(r'([+-]?\s*\d+\.*\d*)\s*(?![a-zA-Z_])', expression)
        ...
        # Retorna a soma dos termos variáveis e os termos constantes
        return lpSum(parsed_terms), sum(float(c.replace(' ', '')) for c in constant_terms) if constant_terms else 0
...
def optimize(request):
    if request.method == 'POST':
        try:
            ...
            objective_expression, objective_constant = parse_expression(objectiveFunction, variables)
            ...
            for i, c in enumerate(constraints):
                ...
                if match:
                    ...
                    lhs_expression, _ = parse_expression(lhs, variables)  # Pega apenas a expressão, ignora a constante

                    if operator == '<=':
                        model += lhs_expression <= rhs
                    elif operator == '>=':
                        model += lhs_expression >= rhs
                    elif operator == '=':
                        model += lhs_expression == rhs
            ...
            objective_result = value(model.objective) + objective_constant
